#initialisation d'un module Journalists
module Journalists
#Array journalistes
...
#Combien y a t'il de handle dans cet array
    def self.length
        puts "COmbien y a t'il de handle dans cet array"
        puts "Il y a #{ARRAY.length} handle dans cet array"
...
#Quel est le handle le plus court de cette liste ?
    def self.min
        ...
        puts "Le handle le plus court est : #{ARRAY.min {|a, b| a.length <=> b.length}}"
    
    end

#Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
    def self.five_size
        puts "quel est le nombre de handle contenant 5 caractères"
        puts "le nombre de handle contenant 5 caractères est de #{ARRAY.count {|x| x.size == 6}}"
    end
...
#Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)
    def self.sizesort
        puts "\nvoici le array trié par taille croissante\n \n"
        puts ARRAY.sort {|a, b| a.length <=> b.length}
    end

#Quelle est la position dans l'array de la personne @epenser ?
    def self.findepenser
        puts "\nLa position de @epenser dans l'array est #{ARRAY.index"@epenser"}\n \n"
    end
